namespace Blaise.Api.Tests.Unit.Services
{
    ...
    public class IngestServiceTests
    {
        ...
        {
            // act and assert
            var exception = Assert.ThrowsAsync<ArgumentNullException>(async () => await _sut.IngestDataAsync(
                null,
                _serverParkName,
                _questionnaireName,
                _tempPath));
            Assert.That(exception.ParamName, Is.EqualTo("The argument 'ingestDataDto' must be supplied"));
        }
        ...
        public void Given_An_Empty_BucketFilePath_When_I_Call_IngestDataAsync_Then_An_ArgumentException_Is_Thrown()
        {
            ...
            // act and assert
            var exception = Assert.ThrowsAsync<ArgumentException>(async () => await _sut.IngestDataAsync(
                _ingestDataDto,
                _serverParkName,
                _questionnaireName,
                _tempPath));
            Assert.That(exception.Message, Is.EqualTo("A value for the argument 'ingestDataDto.BucketFilePath' must be supplied"));
        }
        ...
        public void Given_A_Null_BucketFilePath_When_I_Call_IngestDataAsync_Then_An_ArgumentNullException_Is_Thrown()
        {
            ...
            var exception = Assert.ThrowsAsync<ArgumentNullException>(async () => await _sut.IngestDataAsync(
                ...
                _questionnaireName,
                _tempPath));
            Assert.That(exception.ParamName, Is.EqualTo("ingestDataDto.BucketFilePath"));
        }
